#知识点：列表索引（正反）、切片 [start:end:stride]、列表比较
...
print(items4[0])    #索引从 0 开始，0 是第一个元素
print(items4[4])    #0 到 N-1 是正向索引，N 是元素个数
#正向索引：N-1 是最后一个元素

#反向索引
items4[-1] = 'C++'  #-1 是最后一个元素，可读可写
print(items4[-1])
...
#切片语法 [start:end:stride]，左闭右开（含 start、不含 end）
print(items5[0:5:3])    #从 0 开始，每次跳 3
print(items5[2:6:3])
print(items5[-1:-6:-2]) #stride 为负，从右往左取
print(items5[-1:-5:2])  #start 在 end 右边且 stride 为正，取不到值（空）
print(items5[:3:2])     #start 省略，默认从 0 开始
print(items5[1::3])     #end 省略，默认取到末尾
...
print(itemsd > itemse)    #True 是 1、False 是 0，1 > 0 成立
